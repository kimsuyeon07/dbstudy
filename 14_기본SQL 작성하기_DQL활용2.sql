/*
    ...
    4. 칼럼이나 테이블을 대문자로 작성한다. [통일화]소문자로 작성한다면 항상 소문자로 작성한다. : DB에서 다른 쿼리문으로 보게 된다.
...
-- 1) distinct함수 사용.(중복 제거)
SELECT
      DISTINCT location_id
  FROM departments;
-- 2) GROUP BY 함수 사용.
SELECT
      location_id       -- select절에서 location)id를 사용하려면,
  FROM departments  
...
-- 33. 같은 지역(location_id) 끼리 모아서 해당 지역(location_id) 에 어떤 부서(department_name) 가 있는지 조회한다. (만들 수 없는 쿼리문 형식.)
SELECT
       location_id
     , department_id
  FROM departments
 GROUP BY location_id, department_id; -- 억지로 실행을 만든 쿼리. 사실 문제 자체가 그룹핑이 불가한 상환  
 -- 굳이 그룹화를 할 필요 없는 쿼리문.
...
SELECT
  ...
  FROM employees
 GROUP BY job_id
 ORDER BY AVG(salary);  -- 또는 avg(salary)의 별명을 사용한다. [ order by 급여평균; ] 
-- 실행 순서에 따라서 급여평균이라는 별명을 가진채로 order by절을 실행하기 때문에 사용이 가능하다.
...
SELECT
  ...
  FROM employees
 GROUP BY department_id
HAVING avg(salary) >= 10000;  -- HAVING 평균급여 >= 1000; 은 불가능하다.
